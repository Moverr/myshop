...
if(!empty($area) && in_array($area, array('publish_bidder_lots')))
{
        ...
        $lot_id_info = '';
			...
			# print_array($bebresult);
				...
				$ddate_octhe = display_date($bebresult[0]['ddate_octhe']);
				$num_orb = $bebresult[0]['num_orb'];
				$date_oce_r	=display_date($bebresult[0]['date_oce_r']);
				$date_oaoterbt_cc = display_date($bebresult[0]['date_oaoterbt_cc']);
				$beb_expiry_date =  display_date($bebresult[0]['beb_expiry_date']);
				$final_evaluation_report_approval_date =display_date($bebresult[0]['final_evaluation_report_approval_date']);
                $evaluation_commencement_date = display_date($bebresult[0]['evaluation_commencement_date']);
				...
				$date_beb_expires =  display_date($bebresult[0]['beb_expiry_date']);
				$justification = trim($bebresult[0]['justification']);
				$lot_id_info = !empty($bebresult[0]['received_lot']) ?$bebresult[0]['received_lot'] :$lott_id ; 
                $date_of_display = display_date($bebresult[0]['date_of_display']);    
				...
				//Get me Lot Id Info 	
				 $lot_id_info = !empty($lot_id_info) ? $lot_id_info :$lott_id ; 
...
xc = 0;
	...
	{	
		 ...
		 var read_price = readdata['readoutprice_'+xc];
		     exchanged_rate = readdata['exchangerate_'+xc];

	 		if(!isNaN(read_price))
{
			 
			 
			 if(exchanged_rate ==undefined || exchanged_rate == '' )
			{
			exchanged_rate = 1;
			}
			else
			{
				  if( exchanged_rate.length <= 0)
					{
						exchanged_rate = 1;
					}
					else
					{	
						exchanged_rate =  exchanged_rate.replace(/\D/g, '');
					}
		 ...
		 total_price =  total_price +  parseFloat(parseFloat(exchanged_rate) * parseFloat(read_price));
...
<div >
	        ...
	        <div class="controls">
	         ...
	         <div class="controls">
	         <input class=" m-ctrl-medium date-picker   dob_commencement span6"  data-date-format="dd/mm/yyyy" data-date-viewmode="days"      type="text" value="<?=$ddate_octhe; ?>" id="dob_commencement" name="dob_commencement" />
	         <button type="button" onclick="javascript:   $('#dob_commencement').attr('value','');"><i class="fa fa-refresh"></i> </button>
			 </div>
	         </div>
	         ...
	         <div class="controls">
	         <input class=" m-ctrl-medium date-picker   dob_evaluation span6"   data-date-format="dd/mm/yyyy" data-date-viewmode="days"     name="dob_evaluation" id="dob_evaluation"  type="text" value="<?=$date_oce_r; ?>" />
				<button type="button" onclick="javascript:   $('#dob_evaluation').attr('value','');"><i class="fa fa-refresh"></i> </button>		     
			 </div>
	         </div>
	         ...
	         <div class="controls">
	         <input class=" m-ctrl-medium date-picker  dob_cc span6" data-date-format="dd/mm/yyyy" data-date-viewmode="days"   type="text" value="<?=$date_oaoterbt_cc;?>" id="dob_cc" name="dob_cc" />
			 	<button type="button" onclick="javascript:   $('#dob_cc').attr('value','');"><i class="fa fa-refresh"></i> </button>
		       </div>
	         </div>
					 <?php
    ...
    <?php
     ...
     <div class="control-group div_optional">
                                <label class="control-label "> Display Date of  BEB </label>
                              <div class="controls">
                              <input class=" m-ctrl-medium date-picker   date_of_display span6"   data-date-format="dd/mm/yyyy" data-date-viewmode="days"     name="date_of_display" id="date_of_display"  type="text" value="<?=$date_of_display; ?>" />
									<button type="button" onclick="javascript:   $('#date_of_display').attr('value','');"><i class="fa fa-refresh"></i> </button>
							</div>
	                            ...
	                            <div class="controls">
	                            <input class=" m-ctrl-medium date-picker   date_beb_expires span6"  data-date-format="dd/mm/yyyy" data-date-viewmode="days"      name="date_beb_expires" id="date_beb_expires"  type="text" value="<?=$date_beb_expires; ?>" />
		                         	<button type="button" onclick="javascript:   $('#date_beb_expires').attr('value','');"><i class="fa fa-refresh"></i> </button>
								 </div>
...
<br/>
      ...
      <!-- Provide Justification -->
       <div  style="display:none;" class="row-fluid  justification_div"  >
              <div class="control-group">
		              ...
		              <div class="controls">
				            <textarea class="formcontrol span4 justification" id="justification" >	            	
	...
	<div class="alert alert-info">
      ...
      No Best Evaluated Bidder Selected
			...
			-->
				<button type="submit" name="save" value="save" class="btn blue plishtype " <?php if(!empty($lots)){  ?> onClick="javascript:btn_status(10)" <?php } else { ?>  onClick="javascript:btn_status(1)" <?php } ?> >  Update  <?php if(!empty($lots)){  echo" Lot "; } ?></button>


				
        <?php if(!empty($lots)){ ?>
        	<button type="submit" name="save" value="save" class="btn blue plishtype" onClick="javascript:btn_status(10)" > Save   And Exit </button>
        	 <?php } ?>
         <?php if(!empty($lots)){  ?> 
          	<a  href="<?=base_url()."receipts/manage_bebs"; ?>"    class="btn" >  Exit</a>
           <?php  } ?>


			 <?php if(empty($lots)){  ?>	
			 <button type="submit" name="publish" value="publish" class="btn plishtype" onClick="javascript:btn_status(1)">Update &  Publish </button>
			 <a  href="<?=base_url()."receipts/manage_bebs"; ?>"    class="btn" >  Exit</a>

			  <?php } ?>				
				

				<button type="reset" name="reset" value="reset" class="btn blue resettn "  >  Clear Form </button>
