	...
	function save_bidreceipt(){
		...
		switch ($segment) {
			...
			case 'update':
				...
				log_action('update',' A bid Receipt has been successfully updated  to '.$procurementrefno.' ', ' A bid Receipt has been successfully updated to '.$procurementrefno.' ');
			...
			case 'insert':
				...
				if($result == 1)
				{
					...
					$refence_num = $this->bid_invitation_m->get_bid_invitation_info($_POST['bidid'] ,'procurement_ref_no');

					log_action('create',' A bid Receipt has been successfully added to '.$refence_num.' ', ' A bid Receipt has been successfully added to '.$refence_num.' ');
				}
	...
	function save_disposal_bidreceipt(){
		...
		switch ($segment) {
			...
			case 'insert':
				...
				log_action('create',' A Disposal  Receipt has been successfully added   ', ' A Disposal  Receipt has been successfully for  '.$_POST['serviceprovider'].'  ');
				print_r($result);
				break;
...
//decryptValue
	...
	function delreceipts_ajax()
	{
		...
		log_action('delete',' A   Receipt has been successfully Deleted   ', ' A Receipt  Receipt has been successfully Deleted   ');
	...
	function manage_bebs(){
		...
		if(!empty($data['skip']))
		{
			$data['formdata']['skipdata'] = $data['skip'];
			$data['skipdata'] = explode("::", $data['skip']);
		}
...
//		print_array(count($data['active_bebs']));
//		print_array($data['active_bebs']);
//		exit(print_array($this->db->last_query()));
		...
		$data['view_data']['form_title'] = $data['page_title'];
		$data['search_url'] = 'receipts/search_bebs/level/'.$data['level'].'/financial_year/'.$current_financial_year;
		$this->load->view('dashboard_v', $data);
	...
	function search_bebs()
	{
		...
		$data['current_financial_year'] = !empty($data['financial_year']) ? $data['financial_year'] : currentyear.'-'.endyear;
...
#get awarded contracts  lots
	...
	function get_beb_lots_ajax(){
		...
		if(!empty($result['page_list']))
		{
			...
			foreach ($result['page_list'] as $key => $row) {
				//	print_r($row);
				$st .= '<option value="'.$row['receiptid'].'">'.$row['lot_title'].' </option>';
			}
			print_r($st);
		}
		else {
	...
	function fetchbeb_lots_ajax()
	{
		...
		if(!empty($_POST['lotid']))
		{
			$searchstring = " received_lots.receiptid = '".mysql_real_escape_string($_POST['lotid'])."'";
	...
	function searchproviders()
	{
		...
		if(count($result) >0)
		{
			...
			$sql = "INSERT INTO `tenderpo_itp`.`attempted_beb_award`
 				  ( `userid`, `bidid`, `details`, `dateadded`,`providernames`) VALUES ( ".$userid.", ".$_POST['bidid'].", '".$body."', NOW(),'".$providernames."')";
	...
	/*
	 * Add Admin Review Records to Lots at BEB  View
	 */
	...
	/*
     * Fetch Admin Review for a given lot
     */
