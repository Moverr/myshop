...
//exit(print_array($all_contracts_in_this_year));
...
foreach( $all_contracts_in_this_year as $row){
    ...
    $providers = get_provider_by_receipt_id($row['rct']);

    $row['providers_list'] = $providers[0]['providers'];
    ...
    if(!empty($row['framework']) && $row['framework'] == 'Y')
    {

        # print_r($row['id'].':::');

        $row['call_off_contract_value']  =  get_sum_of_contract_values($row['id']);

        $calloff_contract_value =   $row['call_off_contract_value'][0]['calloff_contract_value'];
        if(!empty($calloff_contract_value)  &&  $calloff_contract_value  > 0 )
        {
            $row['final_contract_value'] =  $calloff_contract_value;
        }
        # $call_off =  get_sum_of_contract_values($row['id']);
        # print_r($calloff_contract_value);
