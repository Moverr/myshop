...
class Bids extends CI_Controller {
    ...
    function validate_ifb_date(){
     ...
     $last_date  =   addDays("2016-06-09",14,$skip_days,$public_holiday);

     echo "Passed Date wAs 2016-06-09 <br/> New Date Is  ";
            ...
            if($validation_results['bool'] && $bid_validation_results['result'])
            {
                ...
                else
                {
					...
					//Exchange Rate  estimated_amount_exchange_rate
					 $_POST['estimated_amount_exchange_rate'] = !empty($_POST['estimated_amount_exchange_rate']) ? removeCommas($_POST['estimated_amount_exchange_rate']) : '';
                    ...
                    if(!empty($data['i']))
                    {
                       ...
                       # exit($this->db->last_query());
                        ...
                        $query = mysql_query("UPDATE   procurement_plan_entries SET total_ifb_quantity ='".$totall."' WHERE  	id='".$_POST['procurement_id']."' limit 1 ") or die("".mysql_error()) ;
						 ...
						 log_action('update','IFB '.$_POST['procurement_ref_no'].'  Record Updated  Successfully  ', 'IFB Record  has been Updated  Successfully');
                    ...
                    else
                    {
                        ...
                        $query = mysql_query("UPDATE   procurement_plan_entries SET total_ifb_quantity ='".$totall."' WHERE  	id='".$_POST['procurement_id']."' limit 1 ") or die("".mysql_error()) ;
						 ...
						 log_action('create','IFB   '.$_POST['procurement_ref_no'].'  Record Created Successfully  ', 'IFB Recrods  has been Created  Successfully');
            ...
            if($validation_results['bool'])
            {
                ...
                if(!empty($result) && $result)
                {
                    ...
                    $this->session->set_userdata('sres', $data['msg']);
                      ...
                      log_action('approve','IFB   '.$data['formdata']['procurement_ref_no'].'    Record Aprroaved Successfully     ', 'IFB   '.$data['formdata']['procurement_ref_no'].'    Record Aprroaved Successfully  ');  
...
//end
    ...
    function delete_bid_invitation()
    {
        ...
        if(!empty($data['i'])){
           ...
           log_action('delete','IFB     Record Deleted  Successfully     ', 'IFB        Record Deleted Successfully  ');  
   ...
   #cancel Bid Invitation 
	function cancel_bid_invitation()
	{
        ...
        $formdata = $_POST ;
            ...
            log_action('cancel','IFB     Record Cancelled  Successfully     ', 'IFB        Record Cancelled Successfully  ');  
        ...
        switch($btnstatus)
        {
            ...
            default:
                ...
                $result = $this-> Receipts_m -> publishbeb($post);
					...
					{
						   log_action('update','BEB  Record Updated  Successfully   ', 'BEB    Record Updated Successfully  ');  
    
					}
					else
					{
						   log_action('create','BEB   Record Created  Successfully   ', 'BEB    Record Created  Successfully  ');  
