...
//    }
    ...
    function get_all_bids($from, $to, $pde = '', $financial_year = '')
    {
        $this->db->query('SET SQL_BIG_SELECTS=1');
        ...
        BI.id,
        IF(BI.procurement_method_ifb > 0, PM.title , PPM.title) as procurement_method_title,
        (SELECT COUNT(*) FROM receipts  WHERE bid_id=BI.id AND isactive="Y"  and (select DISTINCT count(*) from contracts where bidinvitation_id=BI.id and isactive="Y")>0 ) as total_responses,
        (SELECT COUNT(*) FROM receipts WHERE bid_id=BI.id AND receipts.nationality LIKE "Uganda" AND receipts.isactive="Y" and (select DISTINCT count(*) from contracts where bidinvitation_id=BI.id and isactive="Y")>0) as total_local_responses,
        (SELECT DISTINCT bestevaluatedbidder.num_orb from bestevaluatedbidder WHERE bestevaluatedbidder.bidid=BI.id AND bestevaluatedbidder.isactive="Y" and (select DISTINCT count(*) from contracts where bidinvitation_id=bestevaluatedbidder.bidid and isactive="Y")>0 LIMIT 1) AS total_responsive_bids,
        (SELECT DISTINCT bestevaluatedbidder.num_orb_local from bestevaluatedbidder WHERE bestevaluatedbidder.bidid=BI.id AND bestevaluatedbidder.isactive="Y" and (select DISTINCT count(*) from contracts where bidinvitation_id=bestevaluatedbidder.bidid and isactive="Y")>0 LIMIT 1) AS total_local_responsive_bids
