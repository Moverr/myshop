...
#procurement providers
  ...
  function fetchproviders_lots($bidid =0,$searchstring = ' ')
  {
    ...
    $data = array('SEARCHSTRING' => ' and  bidinvitations.id = '.$bidid.$searchstring,'approved'=>'Y');
    ...
    return $result;
      ...
      if(!empty($result))
      {
          ...
          if(((strpos($value['providerid'] ,",")!== false)) &&  (preg_match('/[0-9]+/', $value['providerid'] )))
      	{
              ...
              $st .="<option  data-readoutprice=".$value['read_price']."  data-country=".$value['nationality']."   value=".$value['receiptid'].">".$prvider." &nbsp [JV]  </option>";
      		#print_r($prvider);
      	    }
              ...
              $st .= "<option   data-readoutprice=".$value['read_price']."  data-country=".$value['nationality']."   value=".$value['receiptid']."   >".$records['providernames']."</option>";
      	    }
	...
	//	$ddat = $dd[2].'-'.$dd['1'].'-'.$dd[0];
	  ...
	  $rand2 = rand(90867,62726);
		...
		if(!empty($post['pricing']))
		{
			...
			{
				$prvider = explode('_',$key);
                $query = mysql_query("INSERT INTO  readoutprices(readoutprice,currence,exchangerate,receiptid) values ('".$aty2[$x]."','".$aty[$x]."','".removeCommas($aty3[$x])."','".$insert_id."') ")or die("".mysql_error());
				$x ++;
			}
...
#exit("REACHE1A");
	  ...
	  $readoutprice = isset($post['readoutprice']) ?$post['readoutprice'] : '' ;
		...
		//get bid information ::
			$bidid  = 0;
			$query = mysql_query("SELECT a.* FROM bidinvitations a inner join  procurement_plan_entries b on a.procurement_id = b.id  where a.id = '".$procurementrefno."' limit 1");
	...
	#	print_r($insertid);
		...
		if(!empty($post['pricing']))
		{
			...
			while($x < $length)
			{
				...
				$query =  mysql_query("INSERT INTO  readoutprices(readoutprice,currence,exchangerate,receiptid) values ('".$aty2[$x]."','".$aty[$x]."','".removeCommas($aty3[$x])."','".$insertid."') ");
				#print_r($query); exit();
...
$query = mysql_query(" select a.* from disposal_bid_invitation a    INNER JOIN disposal_record  b   ON a.disposal_record = b.id   where a.id = '".$disposalbidid."' limit 1");
		...
		if(!empty($post['pricing']))
		{
			...
			{
				$prvider = explode('_',$key);
                $query = mysql_query("INSERT INTO  disposalreadoutprices(readoutprice,currence,exchangerate,receiptid) values ('".$aty2[$x]."','".$aty[$x]."','".removeCommas($aty3[$x])."','".$insert_id."') ")or die("".mysql_error());
				$x ++;
			}
	...
	#	print_r($insertid);
		...
		if(!empty($post['pricing']))
		{
			...
			while($x < $length)
			{
				...
				$query =  mysql_query("INSERT INTO  disposalreadoutprices(readoutprice,currence,exchangerate,receiptid) values ('".$aty2[$x]."','".$aty[$x]."','".removeCommas($aty3[$x])."','".$insertid."') ");
				#print_r($query); exit();
	...
	function updatebidreceipt($post,$idx)
	{
		...
		//get bid information ::
			$bidid  = 0;
		$query = mysql_query("SELECT * FROM bidinvitations where procurement_ref_no = '".$procurementrefno."' limit 1");
...
/*
 $issactive = "Y";
      $result = paginate_list($this, $data, 'fetchproviders_pde', array('SEARCHSTRING' => '  and  users.userid ='.$userid.' and receipts.isactive = "'.$issactive.'" order by receipts.dateadded DESC'),10);
      */
  # $query = $this->Query_reader->get_query_by_code('clean_fetch_providers_pde', array('SEARCHSTRING1' => ' AND  bidinvitations.isactive ="Y" AND procurement_plan_entries.isactive ="Y" AND a.isactive ="Y" and bidinvitations.id ="'.$bidid.'"   ','SEARCHSTRING2' => ' and bidinvitations.id ="'.$bidid.'"   '));
  # print_r($query); exit();   
  #   $result = $this->db->query($query)->result_array();
     $result = paginate_list($this, $data, 'clean_fetch_providers_pde', array('SEARCHSTRING1' => ' AND  bidinvitations.isactive ="Y" AND procurement_plan_entries.isactive ="Y" AND a.isactive ="Y" and bidinvitations.id ="'.$bidid.'"   ','SEARCHSTRING2' => ' and bidinvitations.id ="'.$bidid.'"   '),100);
     return $result;
...
/*
       array('SEARCHSTRING1' => ' and procurement_plan_entries.procurement_ref_no ="'.$procurement.'"   ','SEARCHSTRING2' => ' and bidinvitations.procurement_ref_no ="'.$procurement.'"   ') */
          ...
          if($lotid > 0)
          {
            $searchstring = ' and  receipts.bid_id ='.$bidid.' and receipts.receiptid !='.$receiptid.' and lots.id ='.$lotid;
            ...
            $searchstring2 =  ' AND receipts.beb !="Y" and  receipts.bid_id ='.$bidid.' and receipts.receiptid !='.$receiptid.'';
	...
	/*random generator */
		...
		if(!empty($post['dob_commencement']) && ($post['dob_commencement'] != '' ))
		 	$dob_commencement = date('Y-m-d', strtotime($post['dob_commencement']));		 
		else
			$dob_commencement = '';
		...
		$num_bids = removeCommas(mysql_real_escape_string($post['num_bids']));
		 ...
		 if(!empty($post['dob_evaluation']) && ($post['dob_evaluation'] != '' ))
		 	$dob_evaluation = date('Y-m-d', strtotime($post['dob_evaluation']));		 
		 else
			$dob_evaluation = '';
		...
		#$dob_cc = $post['dob_cc'];
		 ...
		 if(!empty($post['dob_cc']) && ($post['dob_cc'] != '' ))
		 	$dob_cc = date('Y-m-d', strtotime($post['dob_cc']));		 
		 else
			$dob_cc = '';
		...
		$date_beb_expires = date('Y-m-d',strtotime($post['date_beb_expires']));
		else
		$date_beb_expires = '';
	
	if(!empty($post['date_of_display']))
		$date_of_display = date('Y-m-d',strtotime($post['date_of_display']));
		else
		$date_of_display = '';
    ...
    $justification   = mysql_real_escape_string($post['justification']);
		...
		$this->session->unset_userdata('bebid');
					...
					if(!empty($post['ifbslot']))
				  {
					  ...
					  {
						  return "3: A BEB Has Been Awarded to this Lot"; 
	...
	foreach ($answerd as $key => $value) {
	  ...
	  #fetch beb list to
        ...
        function fetch_lots_awarded_beb_notincontracts($data,$post)
        {
          ...
          // $arrayName = array('' => , );
          //received_lots.lotid NOT IN (SELECT lotid FROM contracts) AND
          $searchstring = '  received_lots.lotid NOT IN (SELECT lotid FROM contracts) AND    receipts.bid_id='.$bidid ;
          //AND    receipts.bid_id='.$bidid;
            $query = $this->Query_reader->get_query_by_code('fetch_bebs_to_lots');
  ...
  function fetchbeb($data,$searchstring='')
	...
	/*$query = $this->Query_reader->get_query_by_code('fetchbebs', array('SEARCHSTRING' => $searchstring.' and bestevaluatedbidder.ispublished = "Y"  and  receipts.beb="Y" order by bestevaluatedbidder.dateadded DESC','limittext' => '' ));
	 print_r($query); exit(); */
         ...
         $result = paginate_list($this, $data, 'fetchbebs', array('SEARCHSTRING' => $searchstring.' and bestevaluatedbidder.ispublished = "Y"  and  receipts.beb="Y" order by  bestevaluatedbidder.date_of_display  DESC'),10);
		 return $result;

			$issactive = "Y";
		    $result = paginate_list($this, $data, 'fetchbebs', array('SEARCHSTRING' => 'and bestevaluatedbidder.ispublished = "Y"  and  receipts.beb="Y" order by  bestevaluatedbidder.date_of_display  DESC'),10);
			return $result;
   ...
   #cancel Beb Lot :: 
                        ...
                        $bebid  =  mysql_real_escape_string($post['bebid']);
                        
                        $query = $this->db->query("DELETE FROM  bestevaluatedbidder   where id=".$bebid) or die("Manage Bebs : ".mysql_error());
                       
                        $query = mysql_query("UPDATE receipts set beb ='P' where  receiptid in (SELECT receiptid  FROM received_lots where lotid = '".$lotid."'  )  AND bid_id = (SELECT bidid from bestevaluatedbidder where lotid = '".$lotid."' limit 1 )  ")  or die("".mysql_error());
	  ...
	  function manage_beb_wholelots_action($post)
            {
                  ...
                  switch($action){
                        ...
                        case 'cancelbeb':
                        $databidid =  mysql_real_escape_string($post['databidid']);
                        $query = $this->db->query("DELETE FROM  bestevaluatedbidder   where pid in (SELECT receiptid FROM receipts where bid_id =".$bebid.") " ) or die("Manage Bebs : ".mysql_error());
                        $query = $this->db->query("UPDATE receipts SET beb ='P', reason='' where bid_id=".$databidid) or die("Manage Bebs : ".mysql_error());
                        $msg = '1';
                        return $msg;
                        break;
    ...
    function fetchlots($bidid)
      	{
      		$bidid = mysql_real_escape_string($bidid);
      		$query = $this->Query_reader->get_query_by_code('fetch_lots', array('SEARCHSTRING' => 'b.id ="'.$bidid.'" AND b.haslots="Y"      ','limittext' => '','orderby' => ''));
      	  #print_r($query); exit();
      		$result = $this->db->query($query)->result_array();
      		return $result;
       ...
       function fetch_lots_awarded_beb_incontracts($data,$post)
        {
          #print_r($post);
          $bidid = $post['bidid'];
          $searchstring = '  received_lots.lotid  IN (SELECT lotid FROM contracts)  AND receipts.bid_id  = "'.mysql_real_escape_string($_POST['bidid']).'" AND receipts.beb="Y" AND contracts.isactive="Y"  AND bestevaluatedbidder.isreviewed="N"';         
