...
class Page extends CI_Controller {
    ...
    function home()
    {
        ...
        if(!empty($urldata['level']) && ($urldata['level'] == 'search'))
        {
            ...
            $data = paginate_list($this, $data, 'active_ifb_details', array('orderby' => 'bid_dateadded DESC', 'searchstring' => $search_str .
                ' AND procurement_plans.isactive="Y" AND bidinvitations.isactive = "Y" AND bidinvitations.isapproved="Y" AND bidinvitations.bid_submission_deadline>NOW()'),10);
        ...
        else
        {
            ...
            else
            {
                ...
                #  echo "SERVER 22 SIDE TESTING";


                #  $query = $this->query_reader->get_query_by_code('bid_invitation_details',array('orderby' => 'invitation_to_bid_date DESC', 'searchstring' => 'bidinvitations.isactive = "Y"  AND procurement_plans.isactive="Y"  AND procurement_plan_entries.isactive="Y"  AND    IF(bidinvitations.procurement_method_ifb > 0,bidinvitations.procurement_method_ifb  IN("1","2","9")  ,procurement_plan_entries.procurement_method  IN("1","2","9")  )  AND procurement_plans.isactive ="Y" AND bidinvitations.isapproved="Y" ','limittext'=>' '));

                # print_r($query);


                #get available bids
                $data = paginate_list($this, $data, 'active_ifb_details', array('orderby' => 'invitation_to_bid_date DESC', 'searchstring' => 'bidinvitations.isactive = "Y"  AND procurement_plans.isactive="Y"  AND procurement_plan_entries.isactive="Y"  AND    IF(bidinvitations.procurement_method_ifb > 0,bidinvitations.procurement_method_ifb  IN("1","2","9")  ,procurement_plan_entries.procurement_method  IN("1","2","9","11")  )  AND procurement_plans.isactive ="Y" AND bidinvitations.isapproved="Y" '), 10);
            }
        }
    ...
    function disposal_notices()
    {
        ...
        $searchstring .=" 1 = 1 AND disposal_bid_invitation.isactive ='Y' AND disposal_record.isactive ='Y'  AND disposal_plans.isactive ='Y' ";
    ...
    function view_disposal_notice()
    {
        ...
        $searchstring = "   1=1  AND disposal_bid_invitation.id='".$dipsosal_id."'  AND disposal_record.isactive ='Y' AND disposal_record.isactive='Y' AND pdes.isactive='Y'  ORDER BY  disposal_bid_invitation.dateadded DESC ";
