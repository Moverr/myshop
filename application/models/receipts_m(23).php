	...
	function savebidreceipt($post)
	{
		...
		$bidid = $post['bidid'];
		  ...
		  //Bid and Procurement Information 		 
			$query = mysql_query("SELECT a.* FROM bidinvitations a inner join  procurement_plan_entries b on a.procurement_id = b.id  where a.id =  = '".$bidid."' limit 1");
	...
	*/
	...
	function publishbeb($post){
		...
		if(!empty($post['signature_date']))
			$signature_date= date('Y-m-d',strtotime($post['signature_date']));
		else
			$signature_date = '';
		 ...
		 if(!empty($post['ifbslot']))
			{
				  ...
				  */ 
				$lotted_data =   $this->db->query("SELECT * FROM bestevaluatedbidder  INNER JOIN receipts ON  bestevaluatedbidder.pid =  receipts.receiptid  INNER JOIN received_lots ON  receipts.receiptid =  received_lots.receiptid   WHERE   received_lots.lotid = ".$post['ifblot']."")->result_array();
...
$awarded_beb_with_lots = $this->db->query($query)->result_array();
				...
				else
				{
			     ...
			     if( ( (isset($var['bebid'] )) && (!empty($var['bebid'])) ) &&  ( $update_ready == 0 ) ){
				 
						   if(!empty($awarded_beb_with_lots))
							{
							$pid = $awarded_beb_with_lots [0]['pid'];
							
							$this->db->query("UPDATE receipts SET  beb='N' WHERE	receiptid = ".$pid."");
							}
				    
						$awarded_beb_with_lots = '';
							$query = $this->Query_reader->get_query_by_code('updatebeb',  $data);
							$result = $this->db->query($query);		
							//$msg_said  = "1 ";	
				 ...
				 /*
				  * If Not Empty Awarded BEBs Echo  Error 
				  */ 
					if(!empty($awarded_beb_with_lots))
					{
						return "".$msg_said;
		 ...
		 //Update Receipts to BEB Y where receiptid and Bid_id 
	...
	function disposalpublishbeb($post)
	{
		...
		$cc_award_date = date('Y-m-d',strtotime($cc_award_date));
		
		$date_of_evaluation =   !empty($post['date_of_evaluation']) ? date('Y-m-d',strtotime($post['date_of_evaluation'])) : '';
		$date_of_final_evaluation =   !empty($post['date_of_final_evaluation']) ? date('Y-m-d',strtotime($post['date_of_final_evaluation'])) : '';		
		$cc_approval_date =   !empty($post['cc_approval_date']) ? date('Y-m-d',strtotime($post['cc_approval_date'])) : '';
		...
		$bebid ='';
		if((isset($var['editbeb'] )) && (!empty($var['editbeb'])))
		{
			$bebid = $var['editbeb'];
		}
		...
		if(!empty($bebid)){
			...
			#exit($this->db->last_query());
			$this->session->unset_userdata('editbeb');
		}
		else{
	...
	function manage_beb_action($post)
	{
		...
		switch($action)
		{
			...
			case 'cancelbeb':
				...
				$date_contract_terminated = !empty($post['date_contract_terminated']) ? date('Y-m-d',strtotime($post['date_contract_terminated'])) : "" ;  
	...
	function fetchlots($bidd)
	{
		...
		$query = $this->Query_reader->get_query_by_code('fetch_lots', array('SEARCHSTRING' => ' a.bid_id ="'.$bidd.'"  AND b.haslots="Y"      ','limittext' => '','orderby' => ''));
		#print_r($query); exit();
		$result = $this->db->query($query)->result_array();
		return $result;
